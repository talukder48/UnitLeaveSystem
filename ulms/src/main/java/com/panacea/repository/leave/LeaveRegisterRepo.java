...
public interface LeaveRegisterRepo extends JpaRepository<LeaveRegister,Long> {
	 ...
	 @Query(value = "select * from `hr_leave_register` r where r.recomend_by is not null and r.reject_by is null and r.approve_by is not null and `check_out_status` not like '%Returned From Leave%' and r.rank_code not like '1%'", nativeQuery = true)
